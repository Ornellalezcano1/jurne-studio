...
/**
 * Constantes estáticas movidas fuera del componente para optimizar el rendimiento
 * y cumplir con las reglas de dependencias de React Hooks.
 */
...
const App: React.FC = () => {
  ...
  // Estados específicos para el Overlay de Archivo
  const [archiveSearch, setArchiveSearch] = useState('');
  const [archiveFilter, setArchiveFilter] = useState<ProjectType>('all');
  ...
  // Lógica de filtrado y búsqueda para el Overlay
  const archiveResults = useMemo(() => {
    return PROJECTS.filter(p => {
      const matchesSearch = p.title.toLowerCase().includes(archiveSearch.toLowerCase()) || 
                            p.category[lang].toLowerCase().includes(archiveSearch.toLowerCase());
      const matchesFilter = archiveFilter === 'all' || p.type === archiveFilter;
      return matchesSearch && matchesFilter;
    });
  }, [archiveSearch, archiveFilter, lang]);
      ...
      <AnimatePresence>
        {isArchiveOpen && (
          <motion.div 
            ...
            animate={{ opacity: 1 }} 
            exit={{ opacity: 0 }} 
            className="fixed inset-0 z-[150] bg-[#020202] flex flex-col"
          >
            {/* Cabecera Técnica del Overlay */}
            <header className="px-6 py-6 md:px-12 border-b border-white/5 bg-[#020202]/80 backdrop-blur-2xl sticky top-0 z-[160]">
              <div className="max-w-7xl mx-auto flex flex-col gap-6">
                <div className="flex justify-between items-center">
                  <div className="flex items-center gap-4">
                    <JurneLogo className="w-8 h-8" />
                    <div className="flex flex-col">
                      <h2 className="text-xl font-black tracking-tighter uppercase leading-none">WORKS_ARCHIVE</h2>
                      <span className="text-[9px] font-mono text-[#C3FF00] opacity-50 uppercase tracking-widest mt-1">Status: Selective_Access</span>
                    </div>
                  </div>
                  <button 
                    onClick={() => setIsArchiveOpen(false)} 
                    className="w-12 h-12 bg-white/5 border border-white/10 rounded-full flex items-center justify-center hover:bg-[#C3FF00] hover:text-black transition-all group"
                  >
                    <X size={20} className="group-hover:rotate-90 transition-transform" />
                  </button>
                </div>

                {/* Filtros y Búsqueda Internos */}
                <div className="flex flex-col lg:flex-row gap-4 items-center">
                  <div className="relative flex-1 w-full">
                    <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-white/20 w-4 h-4" />
                    <input 
                      type="text" 
                      placeholder={lang === 'es' ? "Buscar por nombre o categoría..." : "Search by name or category..."}
                      className="w-full bg-white/5 border border-white/10 rounded-xl py-3 pl-12 pr-4 text-xs font-bold focus:border-[#C3FF00]/50 outline-none transition-all"
                      value={archiveSearch}
                  ...
                  <div className="flex items-center gap-2 overflow-x-auto no-scrollbar w-full lg:w-auto">
                    {CONTENT.work.filters.map((f) => (
                      <button 
                        key={f.id}
                        onClick={() => setArchiveFilter(f.id as ProjectType)}
                        className={`px-5 py-2.5 rounded-xl text-[9px] font-black uppercase whitespace-nowrap transition-all border ${
                          archiveFilter === f.id 
              ...
              <div className="max-w-7xl mx-auto">
                {archiveResults.length > 0 ? (
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {archiveResults.map((project, i) => (
                      <motion.div 
                        key={project.id} 
                        initial={{ opacity: 0, y: 30 }} 
                        animate={{ opacity: 1, y: 0 }} 
                        transition={{ delay: i * 0.05 }}
                        className="group relative bg-white/[0.02] border border-white/5 rounded-2xl overflow-hidden hover:border-[#C3FF00]/30 transition-all p-6 flex flex-col justify-between aspect-[4/3]"
                      >
                        {/* Indicadores Visuales */}
                        <div className="flex justify-between items-start">
                          <div className="flex flex-col gap-1">
                            <span className="text-[9px] font-mono text-[#C3FF00]">ID://{project.id}</span>
                            <span className="text-[8px] font-black uppercase text-white/20 tracking-[0.2em]">{project.year}</span>
                          </div>
                          <div className="p-2 bg-white/5 rounded-lg group-hover:bg-[#C3FF00] group-hover:text-black transition-all">
                        ...
                        {/* Contenido */}
                        <div className="mt-12">
                          <h3 className="text-2xl md:text-3xl font-black uppercase tracking-tighter mb-2 group-hover:text-[#C3FF00] transition-colors">{project.title}</h3>
                          <div className="flex items-center gap-3">
                            <div className="w-8 h-[1px] bg-[#C3FF00]/30" />
                            <span className="text-[9px] font-black uppercase tracking-widest text-white/40">{project.category[lang]}</span>
                          </div>
                        </div>

                        {/* Overlay Decorativo Background */}
                        <div className={`absolute -right-12 -bottom-12 w-40 h-40 ${project.imageColor} opacity-5 blur-3xl rounded-full group-hover:opacity-20 transition-opacity`} />
                      </motion.div>
                    ))}
                  </div>
                ) : (
                  <div className="h-64 flex flex-col items-center justify-center text-center">
                    <ZapOff className="text-white/10 w-12 h-12 mb-4" />
                    <p className="text-white/40 font-black text-sm uppercase tracking-widest">No_Results_Found</p>
                    <button 
                      onClick={() => { setArchiveSearch(''); setArchiveFilter('all'); }}
                      className="mt-4 text-[#C3FF00] text-[9px] font-black uppercase border-b border-[#C3FF00] pb-1"
                    >
                      Clear Filters
                    </button>
                  </div>
            ...
            {/* Footer Técnico del Overlay */}
            <footer className="p-6 border-t border-white/5 flex justify-between items-center text-[8px] font-mono text-white/20 uppercase tracking-[0.3em]">
              <div className="flex items-center gap-4">
                <span>Displaying: {archiveResults.length}/{PROJECTS.length}</span>
                <span className="hidden md:inline">Sort: Latest_First</span>
              </div>
              <div className="flex items-center gap-2">
                <div className="w-1.5 h-1.5 bg-[#C3FF00] rounded-full animate-pulse" />
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-px bg-white/5 border border-white/5 rounded-3xl overflow-hidden relative z-10">
            {SERVICES.map((service, idx) => (
              <motion.div 
                ...
                transition={{ delay: isMobile ? 0 : idx * 0.15, duration: 0.6 }} 
                viewport={{ once: false, amount: 0.1 }} 
                className="group p-10 md:p-16 min-h-[400px] flex flex-col justify-between hover:bg-[#C3FF00] transition-all duration-500 cursor-pointer" 
                onClick={() => scrollToTop()}
              >
                <div className="flex justify-between items-start">
                ...
                <div className="space-y-4">
                  ...
                  <p className="text-white/40 text-sm group-hover:text-black/70 transition-colors leading-relaxed">
                    {service.description[lang]}
                  </p>
                  <div className="flex flex-wrap gap-2 pt-4">
                    {service.tags[lang].map((tag) => (
                      <span key={tag} className="text-[8px] font-black uppercase tracking-widest px-2.5 py-1 border border-white/10 group-hover:border-black/20 rounded-full group-hover:text-black transition-colors">
                        {tag}
                      </span>
                    ))}
                  </div>
        ...
        <section id="work" className="py-24 md:py-40 px-6 md:px-12">
          <div className="max-w-7xl mx-auto">
            <div className="flex flex-col lg:flex-row lg:items-end justify-between gap-8 mb-16">
              ...
              <div className={`flex ${isMobile ? 'flex-row overflow-x-auto pb-4 gap-2 no-scrollbar' : 'flex-col gap-2 items-end'}`}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              {PROJECTS.filter(p => activeFilter === 'all' || p.type === activeFilter).map((project, idx) => (
